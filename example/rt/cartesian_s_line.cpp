...
int main() {
  ...
  try {
    ...
    std::function<CartesianPosition()> callback = [&, rtCon]() {
      ...
      mat_start << pose_start[0], pose_start[1], pose_start[2], pose_start[4], pose_start[5],
          pose_start[6], pose_start[8], pose_start[9], pose_start[10];
      ...
      if (!cart_s.calculateDesiredValues(time, &delta_s)) {
        ...
        std::array<double, 16> new_pose = {{mat_cur(0, 0), mat_cur(0, 1), mat_cur(0, 2), pos_cur(0),
                                            mat_cur(1, 0), mat_cur(1, 1), mat_cur(1, 2), pos_cur(1),
                                            mat_cur(2, 0), mat_cur(2, 1), mat_cur(2, 2), pos_cur(2),
                                            0, 0, 0, 1}};
        cmd.pos = new_pose;
      } else {
        cmd.setFinished();
