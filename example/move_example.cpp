...
void printInfo(const rokae::EventInfo &info) {
  ...
  print(std::cout, "[运动执行信息] ID:", std::any_cast<std::string>(info.at(ID)),
        ...
        std::any_cast<error_code>(info.at(Error)), std::any_cast<std::string>(info.at(Remark)));
}
...
void spiralMove(rokae::xMateRobot &robot) {
  ...
  rokae::MoveAbsJCommand absjcmd(
      {0.0, 0.22150561307150393, 1.4779577696969546, 0.0, 1.2675963456219013, 0.0});
...
void redundantMove(xMateErProRobot &robot) {
  ...
  robot.setEventWatcher(
      Event::safety, [&](const EventInfo &info) { recoverFromCollision(robot, info); }, ec);
...
void avoidSingularityMove(rokae::xMateRobot &robot) {
  ...
  std::vector<rokae::MoveLCommand> cmds = {
      MoveLCommand({0.66675437164302165, -0.23850040314585069, 0.85182031, -3.1415926535897931,
                    1.0471975511965979, 3.01151}),
      MoveLCommand({0.66675437164302154, 0.15775146321850292, 0.464946, -3.1415926535897931,
                    1.0471975511965979, -2.6885547129789127})};
