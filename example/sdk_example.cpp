 ...
 */
template <WorkType Wt, unsigned short DoF> class CalibrateFrame {
public:
  CalibrateFrame(Robot_T<Wt, DoF> &robot, FrameType type, int point_num, bool is_held,
                 const std::array<double, 3> &base_aux = {})
      : robot_(&robot), type_(type), point_list_(point_num), is_held_(is_held),
        base_aux_(base_aux) {}
 ...
 * @brief 示例 - 标定工具/工件坐标系
 */
template <WorkType Wt, unsigned short DoF> void example_calibrateFrame(Robot_T<Wt, DoF> *robot) {
  int point_count = 4;
 ...
 */
template <WorkType wt, unsigned short dof> void example_basicOperation(Robot_T<wt, dof> *robot) {
  error_code ec;
  // *** 查询信息 ***
 ...
 * @brief 示例 - 打开和关闭碰撞检测
 */
template <unsigned short dof> void example_setCollisionDetection(Cobot<dof> *robot) {
  error_code ec;
